    ...
    <div class="container">
        <div class="row align-items-center">
            ...
            <form id="bookSampleForm" class="row g-3" action="{{ route('user.booksample.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    <label for="prescription" class="form-label">Select Package</label>
                    <select data-placeholder="Select Packages" multiple class="form-control" id="prescription" name="package[]">
                        @foreach ($pro as $row)
                        <option value="{{ $row->name }}">{{ $row->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select data-placeholder="Select Tests" multiple class="chosen-select form-control-lg" id="test" name="test[]" required>
                        @foreach ($test as $row)
                        <option value="{{ $row->name }}">{{ $row->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col">
                    <div class="row justify-content-evenly align-items-center pt-2">
                        ...
                        <div class="col-6">
                            <a href="{{ route('cart.previoushomesamplecollection') }}" class="btn btn-small btn-vdc-orange btn-rounded btn-switch-text float-end -me-3 -mt-3">
                                <span>
    ...
    <tbody id="tableContent">
        ...
        <tr>
            ...
            <td class="text-nowrap">
                ...
                <a href="{{ URL::to('homesamplecollectionremove/' . $row->bid) }}" class="btn btn-danger btn-sm" onClick="return confirm('Are you sure?');"><i class="ti ti-trash"></i></a>
                @endif
            </td>
        </tr>
        @endforeach
